...
open class KotlinFindUsagesHandlerFactoryBase(project: Project) : FindUsagesHandlerFactory() {
        ...
        when (element) {
            ...
            is KtParameter -> {
                if (!forHighlightUsages) {
                    ...
                    if (function != null && isOverridable(function)) {
    ...
    //TODO FIR support super methods
    protected open fun checkSuperMethods(
        declaration: KtDeclaration,
        ignore: Collection<PsiElement>?,
        @Nls actionString: String
    ): List<PsiElement> = listOf(declaration)

    // TODO FIR fully support this method
    protected open fun isOverridable(declaration: KtDeclaration): Boolean = declaration.run {
        val parent = parent
        ...
        if (this !is KtNamedDeclaration) return false

        return this.hasModifier(KtTokens.OPEN_KEYWORD)
    }
}
