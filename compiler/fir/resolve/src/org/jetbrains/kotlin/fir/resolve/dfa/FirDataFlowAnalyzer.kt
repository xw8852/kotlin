    ...
    private fun processReturnsForEachContract(
        ...
        for (effect in effects) {
            ...
            if (lambdaArgumentTypeStatement.exactType.isNotEmpty()) {
                val argumentProjection = iterableType.typeArguments.getOrNull(0)
                val newArgumentType = ConeTypeIntersector.intersectTypes(typeContext, mutableListOf<ConeKotlinType>().apply {
                    ...
                    addIfNotNull((argumentProjection as? ConeKotlinTypeProjection)?.type)
                })

                val newVariance = when (argumentProjection) {
                    is ConeKotlinTypeProjectionIn -> Variance.IN_VARIANCE
                    is ConeKotlinTypeProjectionOut -> Variance.OUT_VARIANCE
                    is ConeStarProjection -> Variance.OUT_VARIANCE // no collections with `T : in...`
                    else -> Variance.INVARIANT
                }
                val newArgumentProjection = newArgumentType.toTypeProjection(newVariance)
                val newIterableType = iterableType.withArguments(arrayOf(newArgumentProjection))
